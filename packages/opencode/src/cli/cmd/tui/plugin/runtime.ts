...
function records(cfg: TuiConfig.Info) {
  if (cfg.plugin_records) return cfg.plugin_records
  ...
  return cfg.plugin.flatMap((item) => {
    ...
    if (!meta) return []
    return [{ item, scope: meta.scope, source: meta.source }]
  })
}
