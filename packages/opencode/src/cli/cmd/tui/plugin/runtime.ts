...
function origins(cfg: TuiConfig.Info) {
  if (cfg.plugin_origins) return cfg.plugin_origins
  if (cfg.plugin_records) {
    return cfg.plugin_records.map((item) => ({
      spec: item.item,
      scope: item.scope,
      source: item.source,
    }))
  }
  if (!cfg.plugin?.length || !cfg.plugin_meta) return []
  return cfg.plugin.flatMap((item) => {
    const meta = cfg.plugin_meta?.[Config.pluginSpecifier(item)]
    if (!meta) return []
    return [{ spec: item, scope: meta.scope, source: meta.source }]
  })
}
  ...
  async function load(api: Api) {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const list = origins(config)
        const plugins = Flag.OPENCODE_PURE ? [] : list
        if (Flag.OPENCODE_PURE && list.length) {
          log.info("skipping external tui plugins in pure mode", { count: list.length })
        }
