...
export namespace Plugin {
  ...
  const BUILTIN = ["op-anthropic-auth@0.1.0"]
  ...
  export function plugins(list: Config.PluginOrigin[] | undefined, pure: boolean) {
    if (pure) return []
    if (Flag.OPENCODE_DISABLE_DEFAULT_PLUGINS) return list ?? []
    return Config.deduplicatePluginOrigins([...BUILTIN_ORIGINS, ...(list ?? [])])
  }
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Plugin.state")(function* (ctx) {
          ...
          const plugins = Plugin.plugins(cfg.plugin_origins, Flag.OPENCODE_PURE)
